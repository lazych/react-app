...
function EditPost() {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (appState.user.username != response.data.author.username) {
            appDispatch({ type: "flashMessage", value: "you dont have the permission to edit this post" })
  ...
  useEffect(() => {
    if (state.sendCount) {
      ...
      async function fetchPost() {
        try {
          const response = await Axios.post(`/post/${state.id}/edit`, { title: state.title.value, body: state.body.value, token: state.token }, { cancelToken: ourRequest.token })
